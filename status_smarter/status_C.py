...
while True:
    ...
    for name in names:
        ...
        if status["failed"] > 0 or status["killed"] > 0:
            ...
            if options.resubmit:
                os.system("crab resubmit -d {} --maxmemory=4500 --maxjobruntime=2000 --siteblacklist=T2_US_MIT".format(cmnd.format(name=name)))
            ...
            if options.auto_resubmit:
                ...
                if max_memory_val != 0 or was_killed:
                    cmd = "crab resubmit -d {} --siteblacklist=T2_US_MIT".format(cmnd.format(name=name))
                    if max_memory_val != 0:
